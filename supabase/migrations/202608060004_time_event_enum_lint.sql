...
begin
  ...
  select type into previous from public.time_entries where user_id=app_user_id and (occurred_at at time zone 'America/Los_Angeles')::date=(now() at time zone 'America/Los_Angeles')::date order by occurred_at desc limit 1;
  ...
  insert into public.time_entries(user_id,type,occurred_at) values(app_user_id,next_type,now()) returning * into created;
